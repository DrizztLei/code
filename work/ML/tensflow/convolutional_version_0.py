...
def main(argv=None):
  ...
  conv1_weights = tf.Variable(
    tf.truncated_normal([4, 4, NUM_CHANNELS, 32],  # 2x2 filter, depth 32.
                        stddev=0.1,
  ...
  tf.histogram_summary("REGULAR" , REGULAR * regularizers)
  ...
  with tf.Session() as sess:
    ...
    temp = eval_in_batches(train_data[0:SIZE - VALIDATION_SIZE,...] ,sess , 2)
    count = 0
    for i in xrange(len(temp)):
      correct = (train_labels[i])
      out = (np.argmax(temp[i]))
      if(int(correct) == int(out)):
        count = count + 1

    print(count)
    print(train_labels.shape[0])
    print('%.1f%%' % (count / train_labels.shape[0] * 100))
