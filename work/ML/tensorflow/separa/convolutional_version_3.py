...
def process_data_label():
    ...
    out_data = np.zeros(shape=[SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNEL],dtype=np.float32)
    out_label = np.zeros(shape=[SIZE, LABEL],dtype=np.int64)

    # print (type(data[0]))

    for x in xrange(len(data)):
        out_label[x][label[x]] = 1
        src_img = data[x].astype(np.float32)
        temp = cv.resize(src=src_img, dsize=(IMAGE_SIZE, IMAGE_SIZE))
        out_data[x] = np.array(temp).resize([IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNEL])
...
def main(argv=None):
    ...
    temp = np.zeros(shape=[TRAIN_SIZE])
    for x in xrange(TRAIN_SIZE):
        temp[x] = np.argmax(a=train_label[x])
    ...
    feed_eval_label = tf.placeholder(dtype=tf.int64, shape=[SIZE - TRAIN_SIZE, LABEL])
    ...
    fc2_weights = tf.Variable(
        initial_value=tf.constant(shape=[256, 6], value=0.001, dtype=tf.float32))

    fc2_biases = tf.Variable(initial_value=tf.constant(shape=[6], value=0.001, dtype=tf.float32))
    ...
    with tf.Session() as sess:
        ...
        train_lable_reshape = train_label
