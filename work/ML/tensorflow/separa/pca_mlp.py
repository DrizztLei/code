...
def main(argv=None):
	...
	data, label = parse_data(SIZE=SIZE, IMAGE_SIZE=IMAGE_SIZE, NUM_CHANNEL=NUM_CHANNEL, LABEL=LABEL,
	                         BASE_DIVIDE=BASE_DIVIDE, pca=PCA)
	...
	data = min_max_normalization(data, -5, 5)
	# data = approximate_normalization(data)
	...
	feed_eval_data = tf.placeholder(dtype=tf.float32, shape=[BATCH_SIZE, PCA])
	feed_eval_label = tf.placeholder(dtype=tf.int64, shape=(BATCH_SIZE, ))

	feed_train_data = tf.placeholder(tf.float32, [BATCH_SIZE, PCA])
	...
	fc1_weights = tf.Variable(
		initial_value=tf.constant(shape=[PCA, 32], value=1, dtype=tf.float32))

	fc1_biases = tf.Variable(initial_value=tf.constant(shape=[32], value=1, dtype=tf.float32))

	fc2_weights = tf.Variable(
		initial_value=tf.constant(shape=[32, 16], value=0.01, dtype=tf.float32))

	fc2_biases = tf.Variable(initial_value=tf.constant(shape=[16], value=0.01, dtype=tf.float32))

	fc3_weights = tf.Variable(initial_value=tf.constant(shape=[16, 6], value=0.01, dtype=tf.float32))

	fc3_biases = tf.Variable(initial_value=tf.constant(shape=[6], value=0.01, dtype=tf.float32))

	def forward(info=None, flag=False):
		info = tf.reshape(info, [-1, PCA])
