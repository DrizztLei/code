...
def process_data_label():
    get_file_name()
    data, label = parse_data(SIZE=SIZE, IMAGE_SIZE=IMAGE_SIZE, NUM_CHANNEL=NUM_CHANNEL, LABEL=LABEL, BASE_DIVIDE=BASE_DIVIDE)
    return data, label
...
def main(argv=None):
    ...
    convolution_weights = tf.Variable(initial_value=tf.truncated_normal(shape=[4, 4, NUM_CHANNEL, 2], seed=SEED,
                                                                        stddev=0.01, dtype=tf.float32))

    convolution_biases = tf.Variable(initial_value=tf.zeros(shape=[2], dtype=tf.float32))

    convolution_weights2 = tf.Variable(initial_value=tf.truncated_normal(shape=[4, 4, 2, 4], seed=SEED,
                                                                         stddev=0.01, dtype=tf.float32))

    convolution_biases2 = tf.Variable(initial_value=tf.zeros(shape=[4], dtype=tf.float32))

    convolution_weights3 = tf.Variable(initial_value=tf.truncated_normal(shape=[2, 2, 4, 1], seed=SEED,
                                                                         stddev=0.01, dtype=tf.float32))

    convolution_biases3 = tf.Variable(initial_value=tf.zeros(shape=[1], dtype=tf.float32))
    ...
    fc1_weights = tf.Variable(
        initial_value=tf.constant(shape=[16, 10], value=0.1, dtype=tf.float32))

    fc1_biases = tf.Variable(initial_value=tf.constant(shape=[10], value=0.01, dtype=tf.float32))

    fc2_weights = tf.Variable(
        initial_value=tf.constant(shape=[10, 6], value=0.01, dtype=tf.float32))
    ...
    def forward(info=None, flag=False):
        ...
        relu3 = tf.nn.relu(tf.nn.bias_add(conv3_bn, convolution_biases3))
        ...
        fc = tf.reshape(pool3, [-1, fc1_weights.get_shape().as_list()[0]])
        ...
        hidden_1_relu = tf.nn.sigmoid(hidden_1_bn)

        hidden_2 = tf.matmul(a=hidden_1_relu, b=fc2_weights) + fc2_biases

        # if flag:
        # hidden_2_relu = tf.nn.dropout(hidden_2_relu, DROP_OUT, seed=SEED)
