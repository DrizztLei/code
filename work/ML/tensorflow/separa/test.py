...
def approximate_normalization(data):
    ...
    width = shape[2]
    print (shape)
    approximate_normalization_data = np.ndarray([size, height, width, 1])
    for s in range(size):
        R = data[s, ::, ::, 2]
        G = data[s, ::, ::, 1]
...
def parse_data(SIZE, IMAGE_SIZE, NUM_CHANNEL, LABEL):
    ...
    if NUM_CHANNEL == 3:
        ...
        TRAIN_DATA = np.ndarray(shape=[SIZE, IMAGE_SIZE, IMAGE_SIZE, 3])
        ...
        for x in xrange(len(FILE)):
            ...
            # mat_show(gray)
            temp = cv.resize(gray, (IMAGE_SIZE, IMAGE_SIZE))
            temp = np.reshape(a=temp, newshape=[IMAGE_SIZE, IMAGE_SIZE, 3])
            a = temp.astype(np.float32)
            TRAIN_DATA[x] = a
            RETURN_TRAIN_LABEL[x][TRAIN_LABEL[x]] = 1
        ...
        TRAIN_DATA, RETURN_TRAIN_LABEL = alignment_data(TRAIN_DATA, RETURN_TRAIN_LABEL, LABEL)
        SIZE = RETURN_TRAIN_LABEL.shape[0]
        ROTATE_DATA = np.ndarray(shape=[SIZE * 4, IMAGE_SIZE, IMAGE_SIZE, 3])
        ROTATE_LABEL = np.zeros(shape=[SIZE * 4, LABEL])
        ...
        for x in xrange(SIZE):
            ...
            ROTATE_LABEL[x * 4 + 2][position] = 1
            ROTATE_LABEL[x * 4 + 3][position] = 1
        approximate_normalization_data = approximate_normalization(ROTATE_DATA)
        np.set_printoptions(threshold='nan')
        return approximate_normalization_data, ROTATE_LABEL
