...
public class UserProcess
{
	...
	public int readVirtualMemory(int vaddr, byte[] data, int offset, int length)
	{
		...
		{
			Collection<MemoryAccess> memoryAccesses = createMemoryAccesses(vaddr, data, offset, length,
					AccessType.READ);
	 ...
	 *         the desired action.
	 */
	private Collection<MemoryAccess> createMemoryAccesses(int vaddr, byte[] data, int offset, int length,
			AccessType accessType)
	{
		...
		while (length > 0)
		{
			...
			returnList.add(
					new MemoryAccess(accessType, data, vpn, offset, Processor.offsetFromAddress(vaddr), accessSize));
			length -= accessSize;
			vaddr += accessSize;
			offset += accessSize;
		}
	...
	protected class MemoryAccess
	{
		...
		public int executeAccess()
		{
			...
			if (translationEntry.valid)
			{
				...
				{// Do a read
					System.arraycopy(Machine.processor().getMemory(),
							pageStart + (Processor.pageSize * translationEntry.ppn), data, dataStart, length);
					translationEntry.used = true;
					return length;
				}
				else if (!translationEntry.readOnly && accessType == AccessType.WRITE)
				{
					System.arraycopy(data, dataStart, Machine.processor().getMemory(),
							pageStart + (Processor.pageSize * translationEntry.ppn), length);
