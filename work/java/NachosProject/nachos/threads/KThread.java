...
public class KThread
{
	...
	/**
	 * Wake all the threads that have joined to this thread and remove them from
	 * this threads joined threads set.
	 */
	private void unjoin()
	{
		// Make sure that this thread is being called by the current finished
		// thread
		Lib.assertTrue(
				currentThread == this && currentThread.status == statusFinished && toBeDestroyed == currentThread);
	...
	public static void selfTest()
	{
		...
		selfJoin.setTarget(new Runnable()
		{
			public void run()
			{
				...
				System.out.println(testStatus + ": to it self ");
			}
		});
		selfJoin.fork();
		selfJoin.join();
