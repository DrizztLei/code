...
convex <- tryCatch({
    read.csv(convexHullInfo, head = FALSE, sep = ',')
}, error = function(e){return(data.frame(c(), c()))})
# convex <- read.csv(convexHullInfo, head = FALSE, sep = ',')
...
if(length(convex) != 0)
{
    convex_x <- convex[[1]]
    convex_y <- convex[[2]]
    for(item in seq(1, length(convex_x), 2))
    {

        start_x <- convex_x[item]
        start_x <- get_real_value(mean_x, max_x, min_x, start_x)

        start_y <- convex_y[item]
        start_y <- get_real_value(mean_y, max_y, min_y, start_y)

        end_x <- convex_x[item+1]
        end_x <- get_real_value(mean_x, max_x, min_x, end_x)


        end_y <- convex_y[item+1]
        end_y <- get_real_value(mean_y, max_y, min_y, end_y)

        # lines(c(start_x, start_y), c(end_x, end_y))

        segments(start_x, start_y, end_x, end_y, col = colConvex, lwd = lwdConvex)
    }
}
dev.off()
