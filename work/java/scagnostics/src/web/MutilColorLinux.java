...
public class MutilColorLinux
{
	...
	public static final String root = "/mnt/gentoo/Users/Elvis Lei/Desktop/ADD&DEL/ADD_OUTPUT/";

	public static String path = root; // + "/Stringy/"; // change
										// father
										// directory
	...
	public static void main(String[] args) throws FileNotFoundException
	{
		...
		String[] new_measure =
		{ "/Clumpy/", "/Outlying_MST/", "/Skewed/", "/Stringy/" };

		for (int index = 0; index < new_measure.length; index++)
		{
			System.out.println("building directory : " + new_measure[index]);
			path = root + new_measure[index];
			TxTInfo = path + "/info";
			...
			for (int i = 0; i < feature_number; i++)
			{
				 ...
				 * System.out.println(); for(int j = 1; j < NUMBER; j+=
				 * TYPENUMBER) { System.out.println(filename[j]); }
				 * 
				...
				for (int j = 0; j < varSet.length; j += TYPENUMBER)
				{
					 ...
					 * System.out.print(filename[k + (j / TYPENUMBER) * NUMBER]
					 * + "\t"); } System.out.println();
					 * 
					 * for(int k = 0; k < NUMBER; ++k) {
					 * System.out.print(filename[k + (seq / TYPENUMBER) *
				...
				for (int j = 0; j < size / NUMBER; j++)
				{
					...
					for (int l = 0; l < TYPENUMBER; l++)
					{
						...
						for (int k = 0; k < iteratorTimes; k++)
						{
							...
							sb.append(
									"<td class=\"text-center\" style=\"padding:0 20px;\"><a target=\"_blank\"  href=\"../"
											+ "/" + fatherDirectory + "/" + directory + "/png/" + name
											+ ".png\"><img src=\"../" + "/" + fatherDirectory + "/" + directory
											+ "/png/" + name + ".png\" /></a><p style=\"text-align:center;\">" + data
											+ (k == 0 ? ("</a><p style=\"text-align:center;\">" + varData) : "")
											+ "</td>");
