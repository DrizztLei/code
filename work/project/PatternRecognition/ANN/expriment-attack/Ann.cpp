...
double Ann<T> :: get_regular(){
    ...
    //cout << "Regular return for : " << regular << endl;
    return regular;
}
...
void Ann<T> :: train(T ** const set , int const value , int const times){
    ...
    double loss = value;
    string filename = "";
    for (int i = 0; i < times || loss > 0.35; ++i)
    {
        ...
        if(i % 1000 == 0){
            ...
            loss = softmax(set , point , max , argc_output , value, true);
        }else{
...
void Ann<T> :: init(int input , int argc , int * detailed , int output){
    ...
    for (int i = 0; i < info[argc_level - 1]; ++i)
    {
        ...
        for (int j = 0 ; j < argc_output; ++j)
        {
            ...
            weight[argc_level][i][j] = gaussrand();
