    ...
    ui(new Ui::AddProprietorWidget)
{
    ...
    /*------------------------------------------*/
    this->BUILDING.setCommit(false);
    this->BUILDING.setFlag(false);
    this->BUILDING.setLength(0);
    this->BUILDING.postRequest("select building_id from building");

    QObject::connect(BUILDING.getReply() , &QNetworkReply::finished , [&]()
    {
        qDebug() << "BUILDING .";
        qDebug() << BUILDING.getInfo();
    });
    /*------------------------------------------*/
    ...
    /*-------------------------------------------*/
    this->ID.setCommit(false);
    this->ID.setFlag(false);
    this->ID.setLength(0);
    this->ID.postRequest("select id,name,building_id, room_id,own_date from proprietor");

    QObject::connect(this->ID.getReply() , &QNetworkReply::finished , [&]()
    {
        qDebug() << "ID . ";
        qDebug() << this->ID.getInfo();
    });
    /*-------------------------------------------*/
    ...
    QString material = ui->buildingComboBox->currentText();
    ...
    /*------------------------------------------*/
    this->ROOM.setCommit(false);
    this->ROOM.setFlag(false);
    this->ROOM.setLength(1);
    this->ROOM.addParameter(material);

    QObject::connect(this->ROOM.getReply() , &QNetworkReply::finished , [&]()
    {
        qDebug() << "ROOM .";
        qDebug() << this->ROOM.getInfo();
    });
    ...
    /*----------------------------------------*/
    this->AREA.setCommit(false);
    this->AREA.setFlag(false);
    this->AREA.setLength(0);
    this->AREA.postRequest("select building_id, room_id, area from room where stayed = false");

    QObject::connect(this->AREA.getReply() , &QNetworkReply::finished , [&]()
    {
        qDebug() << "AREA .";
        qDebug() << this->AREA.getInfo();
    });
    /*----------------------------------------*/
}
