    ...
    ui(new Ui::MainWindow)
{
    ...
    getHTTP().setCommit(false);
    getHTTP().setLength(1);
    getHTTP().addParameter(loginId);
    getHTTP().setFlag(false);
    getHTTP().postRequest(sql);

    QNetworkReply* reply = getHTTP().getReply();

    QObject::connect(reply , &QNetworkReply::finished , this , [&]()
    {

        // qDebug() << MainWindow::getHTTP().getInfo();
