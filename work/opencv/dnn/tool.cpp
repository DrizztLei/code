...
double cv_string_to_double(string num);
void cv_resize(Mat & frame , int x , int y , Mat & out);
void cv_display(Mat frame , string display);
...
void thresh_callback(cv::Mat&, int, cv::RNG &);
...
void cv_show_rect(Mat & src_gray , const string & threshold , const string & source , int * thresh , int max_thresh , cv::RNG & rng)
{
    ...
    thresh_callback(src_gray, *thresh, rng);
}


void thresh_callback(cv::Mat & src_gray , int thresh , cv::RNG & rng)
{
